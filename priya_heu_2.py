...
class OffensiveAgent(CaptureAgent):
    def registerInitialState(self, gameState):
        ...
        self.lastDeathPosition = None
        width = gameState.data.layout.width
        height = gameState.data.layout.height
        self.deathMap = [[0 for _ in range(height)] for _ in range(width)]



    def chooseAction(self, gameState):
        # At the start of chooseAction()
        current_position = gameState.getAgentPosition(self.index)

        # Detect respawn (died and returned to start)
        # Detect respawn and record death
        if (
            self.last_position is not None and
            current_position == self.start_position and
    ...
    def getFeaturesForage(self, my_pos, food_list, ghost_positions, capsule_list, gameState, action, carry_food):
        ...
        death_penalty = self.deathMap[x][y]
        features["deathZoneMapPenalty"] = death_penalty
        ...
        # if self.lastDeathPosition:
        #     dist_to_death = self.getMazeDistance(my_pos, self.lastDeathPosition)
        #     if dist_to_death <=5:  # Only penalize if somewhat close
        #         features["recentDeathPenalty"] = 1.0 / (dist_to_death + 1)
                
            # food_dists = sorted([self.getMazeDistance(my_pos, food) for food in food_list])

            
            # features["invFoodDistance"] = 1.0 / (food_dists[1] + 1)
