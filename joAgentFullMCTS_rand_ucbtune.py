...
class MCTSNode:
    ...
    def uctSelectChild(self):
        ...
        def ucb1_tuned_score(child):
            ...
            max_reward = 100
            squared_differences = [(reward - average_value) ** 2 for reward in child.rewards]
            variance = sum(squared_differences) / (child.visits + 1e-4) if child.visits > 0 else 0

            normalized_rewards = [min(reward, max_reward) for reward in child.rewards]
            normalized_average = sum(normalized_rewards) / len(normalized_rewards) if len(normalized_rewards) > 0 else 0            
            exploration = C * math.sqrt(
                (math.log(self.visits + 1) / (child.visits + 1e-4)) * 
                min(0.25, variance + (2 * math.log(self.visits + 1) / (child.visits + 1e-4)))
            )

            return normalized_average + exploration

        return max(self.children, key=ucb1_tuned_score)
