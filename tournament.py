...
def get_agent_files(folder_path,  leaderboard_f, match_history_f):
    ...
    all_elo_scores = choose_combination(agents, leaderboard_f, match_history_f)
...
def choose_combination(agents,   leaderboard_file, match_history_file):
    pd.DataFrame(columns=["match_number", "red_agent", "blue_agent", 
                        "elo_red_after", "elo_blue_after"]).to_csv(match_history_file, index=False)
...
def main():
    ...
    # Run tournament
    get_agent_files(agents_dir, leaderboard_file, match_history_file)
