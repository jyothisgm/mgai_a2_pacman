...
def get_agent_files(folder_path,  leaderboard_f, match_history_f, num_games):
    agents = []
    for file in os.listdir(folder_path):
        if file.endswith(".py"):
            agents.append(os.path.splitext(file)[0]) 
    
    all_elo_scores = choose_combination_(agents, num_games,  leaderboard_f, match_history_f)
...
def choose_combination_(agents, num_games,  leaderboard_file, match_history_file):
    ...
    for red, blue in matchups:
        ...
        scores = run_match(red, blue, num_games)
...
def run_match(red_file, blue_file,  num_games=2):
...
def main():
    ...
    # Run tournament
    agent_tournament = get_agent_files(agents_dir, leaderboard_file, match_history_file,num_games=args.games)
