...
class AttackerAgent(BaseStrategyAgent):
    ...
    def simulateMonteCarlo(self, gameState, depth):
        ...
        value = 0
        last_position = simulationState.getAgentState(self.index).getPosition()
        ...
        for i in range(depth):
            ...
            new_position = simulationState.getAgentState(self.index).getPosition()
            ...
            if currentState and not simulationState.getAgentState(self.index).isPacman and (self.getScore(simulationState) - self.currentScore):
                value += .2 * (depth - i)
                break
            if simulationState.getAgentState(self.index).getPosition() == self.homeBase:
                value -= 10
                break
            if new_position in visited_positions:
                value -= 2  # You can tune this penalty
            visited_positions.add(new_position)

            # if previousDirection == Directions.REVERSE[next_move]:
            #     value -= 10
        value /= (i + 1)
